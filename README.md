...
프로젝트 상세 화면의 워크플로우 카드에서 활성 단계가 있는 흐름은 편집 화면으로 이동하지 않고 바로 실행할 수 있습니다.

워크플로우 편집 화면에서는 실행 버튼과 단계 추가 영역이 오른쪽 사이드바에 있으며, 넓은 화면에서는 스크롤 중에도 사이드바가 고정됩니다.
...
단계가 0이 아닌 종료 코드를 반환하거나 타임아웃되면 해당 단계가 실패하고 나머지 단계는 실행되지 않습니다. 실행 취소 후에는 하위 프로세스 그룹 종료를 최대 2초간 기다리며, 종료 이벤트나 출력 pipe가 남아 있어도 취소를 확정하고 프로젝트 잠금을 해제합니다. 취소 처리 중인 프로젝트에는 새 워크플로우를 시작할 수 없으며, 서버가 그 전에 재시작되면 해당 실행은 `interrupted`로 복구됩니다. 프로젝트 하나에서는 동시에 하나의 워크플로우만 실행할 수 있습니다.
...
- 프로세스 종료·출력 pipe가 지연된 실행의 취소 및 재실행
- 내부 네트워크 IP 접근 제한
...
서버 시작 시 `queued`, `running` 또는 `canceling` 상태로 남아 있는 실행과 단계는 `interrupted`로 정리됩니다. `waiting_input` 실행은 프로세스를 유지하지 않는 안전한 대기 상태이므로 그대로 보존됩니다. 안전하게 백업하려면 서버를 멈춘 뒤 데이터 디렉터리 전체를 복사하세요.
